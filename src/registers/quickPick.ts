            ...
            quickPick.onDidChangeSelection(selection => {
                if (selection.length > 0) {
                    ...
                    if (this.registerManager.hasContent(selected.registerNumber)) {
                        ...
                        this.printOutput(`Selected register ${selected.registerNumber}: ${this.registerManager.getPreview(content, 20)}`);
                        quickPick.hide();
                        resolve(content);
                    } else {
            ...
            quickPick.onDidChangeValue((value) => {
                ...
                if (value.length === 1 && /^[1-5]$/.test(value)) {
                    ...
                    if (this.registerManager.hasContent(registerNumber)) {
                        ...
                        if (content) {
                            this.printOutput(`Keyboard shortcut: Selected register ${registerNumber}: ${this.registerManager.getPreview(content, 20)}`);
                            quickPick.hide();
                            resolve(content);
                            return;
                        }
